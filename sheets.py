...
def export_to_sheets(data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        # --- Головна зміна: підтримка декількох точок розвантаження ---
        unload_points = data.get("unload_points")
        if unload_points and isinstance(unload_points, list) and len(unload_points) > 0:
            # Для кожної точки розвантаження — окремий рядок
            for point in unload_points:
                unload_contact = point.get("unload_contact", "—")
                ...
                values_by_header = {
                    ...
                    "Контакт на завантаженні": _parse_contact(data.get("load_contact", "—"))[0],
                    "Номер телефона на завантаженні": _parse_contact(data.get("load_contact", "—"))[1],
                    "Населений пункт розвантаження": point.get("unload_city", "—"),
                    "Склад розвантаження": point.get("unload_place", "—"),
                    "Спосіб розвантаження": point.get("unload_method", "—"),
            ...
            # Старий режим — одна точка розвантаження
